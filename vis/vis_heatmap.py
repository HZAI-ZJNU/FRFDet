...
class YOLOHeatmap:
    def __init__(
        ...
        target = YOLOTarget(backward_type, conf_threshold, ratio)
    ...
    def process(self, img_path, save_path):
        ...
        img = letterbox(img, self.new_shape)[0]
...
class Args(object):
    ...
    def __init__(self):
        ...
        self.parser.add_argument('--method', type=str, default='GradCAM')
        self.parser.add_argument('--layer', nargs='+', type=int, help='a list of integers')
        self.parser.add_argument('--backward-type', type=str, default='all')
        self.parser.add_argument('--iou-threshold', type=float, default=0.7)
        self.parser.add_argument('--conf-threshold', type=float, default=0.25)
        self.parser.add_argument('--ratio', type=float, default=0.02)
        self.parser.add_argument('--show-box', action='store_true')
        self.parser.add_argument('--renormalize', action='store_true')
        self.parser.add_argument('--new-shape', type=self.str2tuple, default='640x640')
    ...
    def str2tuple(v):
        try:
            ...
            raise argparse.ArgumentTypeError("Tuple must be a comma-separated list, e.g., '224x224'.")

if __name__ == '__main__':
    args = Args()
    model_afpn = YOLOHeatmap(**vars(args.opts))
    model_afpn(args.opts.dataset_dir, args.opts.save_dir)
